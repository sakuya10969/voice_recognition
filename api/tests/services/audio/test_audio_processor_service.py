...
class TestAudioProcessorService:
    ...
    @pytest.fixture
    def service(self, mock_speech_client, mock_blob_client, mock_mp4_processor, mock_transcribe_service):
    ...
    @pytest.mark.asyncio
    async def test_process_audio_success(self, service, mock_blob_client, mock_mp4_processor, mock_transcribe_service):
        """正常系: 音声処理が成功するケース"""
        # 実行
    ...
    @pytest.mark.asyncio
    async def test_process_audio_mp4_fail(self, mock_speech_client, mock_blob_client, mock_transcribe_service):
        """異常系: MP4処理が失敗するケース"""
        # モックの設定
